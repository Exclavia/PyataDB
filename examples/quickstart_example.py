...
if __name__ == "__main__":
    ...
    # If statement checks to see if database file was already created and saved,
    ...
    # I would recommend first loading and checking the file. Which we will do now:
    db.load(filename=db_file, verbose=True)

    # Since we reloaded the database from the file, we have to re-declare our products variable, otherwise it will come back blank.
    ...
    # we could search based on item category, if we wanted a specific item,
    # we could search the name in this case because all the product names happen to be unique, however this is not always the case.
